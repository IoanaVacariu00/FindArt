...
const {JWT_SECRET} = require('../config/keys.cjs')
      ...
      .then(hashedpassword=>{
            ...
            .then(user=>{
                transporter.sendMail({
                    to:user.email,
                    from:"no-reply@findart.com",
                    subject:"signup success",
                    html:"<h1>Welcome to Findart! </h1>"
                })
...
router.post('/reset-password',(req,res)=>{
     crypto.randomBytes(32,(err,buffer)=>{
         ...
         .then(user=>{
             if(!user){
                 return res.status(422).json({error:"There's no account for this email address"})
             }
